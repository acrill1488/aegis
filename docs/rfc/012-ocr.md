...
The active production provider is `UnlimitedOCRProvider`, backed by the `baidu/Unlimited-OCR` model behind an Ubuntu/Docker HTTP service. Windows AEGIS calls the service over HTTP and normalizes responses into `OCRResult`.

PaddleOCR, Tesseract, Vision Language Models, Qwen-VL, UI Graph, Memory, and Companion integrations are not part of this OCR sprint.
